...
1. Enter the project's directory on your control node
...
These techniques are used to manage very large deployments in commercial situations (e.g. in enterprise data centres, telecommunications networks and large-scale internet services), enable and go hand-in-hand with modern development practices such as [CI/CD](https://www.redhat.com/en/topics/devops/what-is-ci-cd), [blue-green deployments](https://martinfowler.com/bliki/BlueGreenDeployment.html) and [canary releases](https://martinfowler.com/bliki/CanaryRelease.html).
...
1. Log in to the control node and run the `upgrade.sh` script:
...
3. The `upgrade.sh` script will now pull up-to-date code from the rollyourown.xyz repositories, build new images for the selected components and deploy them to the host server.
